...
class rmpro(rmmini):
    ...
    def check_frequency(self) -> t.Tuple[bool, float]:
        """Return True if the frequency was identified successfully, and the frequency in MHz."""
    ...
    def find_rf_packet(self, frequency: float = None) -> None:
        ...
        Args:
            frequency: Optional frequency in MHz (e.g., 433.92). If provided,
                      the device will tune to this specific frequency instead
                      of using the frequency found during sweep.
        """
        payload = bytearray()
        if frequency:
